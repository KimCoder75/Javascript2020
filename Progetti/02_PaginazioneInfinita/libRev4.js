...
const blog = {
  ...
  paddingBottomW: 0,
  paddingTop: 30,
  firstPageYCoordinate: [(document.documentElement.clientHeight+1)],
...
function setYCoordinates(scrollHeight) {
  ...
  // TODO: correggere il problema dell'indicatore
  (blog.firstPageYCoordinate[0] === (document.documentElement.clientHeight+1)) ?
...
function miniDebug(){
  console.log('clientHeight==scrollHeight:',document.documentElement.clientHeight==document.documentElement.scrollHeight);
  console.log('blog.posts.length:',blog.posts.length);
  console.log('(clientHeight - firstPageYCoordinate[0] - paddingTop) > 0:',(document.documentElement.clientHeight - blog.firstPageYCoordinate[0] - blog.paddingTop) > 0);
  console.log('blog.firstPageYCoordinate[0]:',blog.firstPageYCoordinate[0]);
  console.log('(clientHeight - firstPageYCoordinate[0] - paddingTop):',(document.documentElement.clientHeight - blog.firstPageYCoordinate[0] - blog.paddingTop));
...
function scrollingAdjustment() {
  let clientHeight, delta, firstPageYCoordinate, paddingTop;
  ({
    clientHeight
  } = document.documentElement);
  ({
  ...
  if (clientHeight - firstPageYCoordinate[0] - paddingTop < 0) {
    delta = 0;
  }
